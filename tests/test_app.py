...
def test_refund_reverses_loyalty_and_inventory(client):
    ...
    c.post(f'/invoices/pay/{inv_id}', data={'amount':'105','payment_method':'Cash'})
    with salon.app.app_context():
        assert salon.Invoice.query.get(inv_id).payment_status == 'Paid'
        before = salon.InventoryItem.query.get(item_id).stock_qty
    c.post(f'/invoices/refund/{inv_id}', data={'amount':'105','refund_method':'Cash','reason':'Test refund'}, follow_redirects=True)
    with salon.app.app_context():
